...
public class PlayerStatisticServiceImpl implements PlayerStatisticService {
    ...
    private final List<String> LEGENDARY_CHARACTER_IDS = Arrays.asList("GRANDADMIRALTHRAWN", "EMPERORPALPATINE", "R2D2_LEGENDARY", "BB8", "COMMANDERLUKESKYWALKER",
            "REYJEDITRAINING", "GRANDMASTERYODA", "CHEWBACCALEGENDARY", "C3POLEGENDARY", "JEDIKNIGHTREVAN", "DARTHREVAN", "DARTHMALAK", "PADMEAMIDALA", "GENERALSKYWALKER");
    ...
    public PlayerStatisticDTO getPlayerStatistic(final Integer playerAllyCode) {
        ...
        final Optional<RosterDTO> playerGuildInfo = guildInfo.getRoster().stream().filter(roster -> playerAllyCode.equals(roster.getAllyCode())).findFirst();
